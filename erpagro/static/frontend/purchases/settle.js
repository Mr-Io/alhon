...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let invoice_pdf_anchor  = document.querySelector("#settlement-pdf-anchor")

    // functions 
    function compute_total_amount_text() {
        let total = 0;
        submit_input.setAttribute("disabled", ""); 
        document.querySelectorAll(".pk-checkbox").forEach(checkbox => {
            if (checkbox.checked) {
                submit_input.removeAttribute("disabled");
                total += parseFloat(checkbox.dataset.baseAmount.replace(',', '.'));
            }
        })
        total_amount_text.innerHTML = `Importe: ${total.toLocaleString("es", number_options)} €`;
    }

    // events
    if (invoice_pdf_anchor){
        window.open(invoice_pdf_anchor.getAttribute("href"));
    }
